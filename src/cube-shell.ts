...
export function buildCubeShellGeometry(level:Level) {
  const n=level.cube!.size,half=n*CELL/2,triangles:number[]=[],iceTriangles:number[]=[];
  ...
  const solid=(x:number,z:number)=>!level.bridges?.some(b=>b.x===x&&b.z===z)&&!['~','#'].includes(level.map[z]?.[x]??'#');
  for(let z=0;z<n;z++)for(let x=0;x<n*6;x++){
    ...
    const icy=!!level.ice?.some(i=>i.x===x&&i.z===z),surface=icy?iceTriangles:triangles;
    for(const v of [outer[0],outer[1],outer[2],outer[0],outer[2],outer[3]])surface.push(...v.toArray());
  ...
  return {surface:geometry(triangles),iceSurface:geometry(iceTriangles),fineEdges:geometry(fine),rimEdges:geometry(rim)};
}
